...
## FILOSOFIA DESTA TELA (leia antes de codar)

Esta tela é uma **bridge page** (página de transição), não uma página de oferta. Isso é uma distinção crítica:

- **O quiz já vendeu.** A pessoa acabou de contar a história de quem ama, escolher o estilo, imaginar a música. Ela já está emocionalmente comprometida.
- **A tela de conversão não vende de novo.** Ela honra o estado emocional que o quiz construiu, resolve a última objeção (player de áudio = prova de qualidade), coleta os dados de entrega e transiciona para o checkout.
- **Cada elemento a mais aqui é fricção.** Stack de valor, depoimentos extras, links para fora — tudo isso gera objeções que o quiz já superou. A regra de ouro: **se o quiz fez o trabalho, não refaça aqui.**

Por isso removemos bullets, depoimentos e saídas. Não porque são "ruins" — porque **esta não é a página deles**.
...
**Regra:** Manter a mesma tag `<p>`, mesmas classes Tailwind, mesmo posicionamento. Só trocar o texto interno. Usar `resolverGenero('[ELA_ELE]', data.genero, data.nome)` para manter o token dinâmico de gênero. Se o resolver retornar "ela", a frase fica: "Daqui a pouco ela vai apertar o play. E vai ouvir, pela primeira vez, uma música que só existe por causa da história de vocês."

**Por que esta frase:** A versão anterior ("vai saber que foi você quem fez isso") é sobre reconhecimento. Esta é sobre o MOMENTO SENSORIAL — apertar o play, ouvir pela primeira vez. Faz a pessoa imaginar a cena em vez de processar um conceito. "Só existe por causa da história de vocês" reforça que não é música genérica — é única porque o quiz capturou a história dela.
...
**Problema:** 4 bullets de valor + H3 "A música que só [A_O] [NOME] vai ter" ocupam espaço considerável antes do preço. **Esta não é uma página de oferta — é uma bridge page.** O quiz já construiu o valor emocional completo. A pessoa que chegou via quiz já sabe o que está comprando — mostrar bullets de "o que você recebe" aqui gera objeção desnecessária ("será que vale?", "o que mais vem?"). O H3 é redundante com a headline principal da tela. O ícone music_cast no meio fragmenta visualmente sem agregar. O separador com linhas gradientes também sai.

**Por que remover:** Na estrutura validada por especialistas (Elton Cipriano, Helena de Guide), 80% da mensagem vende o valor e 20% fecha a venda. **O quiz já fez os 80%.** Esta tela é o 20% — coleta dados e transiciona. Colocar stack de valor aqui é como vender de novo para quem já decidiu comprar. Gera fricção, não elimina.
